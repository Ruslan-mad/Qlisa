...
Qlisa — развиваемый сообществом форк [Inkue от FonograF](https://github.com/FonograF/Inkue). Новые проекты используют `.qlisa`; старые проекты `.inkue` тоже поддерживаются, оба расширения используют одну JSON-схему. Qlisa 1.5.8 включает ассоциацию файлов `.qlisa` в Windows и является последним релизом.
