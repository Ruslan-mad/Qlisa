...
- Settings runtime reinstallation passed on the installed 1.5.6 build. Clicking Reinstall in Preferences relaunched Qlisa and rewrote the runtime files under `%TEMP%\qlisa-settings-candidate-156\LocalAppData\Qlisa\runtime` at 00:48:19–00:48:22. The `.reinstall-pending` marker was absent after restart, and Verify Integrity reported all components as Installed. Verified SHA-256 prefixes: FFmpeg `288EF710…`, ffprobe `8603CD025…`, and libmpv `751131F81…`. Commit `7e68180` grants restart permission to the standalone Preferences window.
