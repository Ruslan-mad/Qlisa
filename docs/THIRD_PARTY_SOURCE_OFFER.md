...
Isolated smoke testing of the 20260923 candidate passed video and audio-track playback (`ao=wasapi` configured; physical audio output unverified), seek, two concurrent libmpv contexts, an actual SRT localhost loopback, and FFmpeg MP4-to-WAV conversion. App-level conversion through `ffprobe.exe` remains untested because this release has no `ffprobe.exe`. These tests do not replace the current runtime; no runtime switch was made.
