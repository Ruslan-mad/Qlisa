...
Qlisa is a community-maintained derivative of [Inkue by FonograF](https://github.com/FonograF/Inkue). In the current source checkout, new projects use `.qlisa` and Save As offers `.qlisa` by default. An explicitly chosen legacy `.inkue` suffix is preserved. Both project extensions use the same JSON workspace schema. The `.qlisa` Windows file association is planned for a future Qlisa release and is not in the released 1.5.6 installer.
